...
.PHONY: build-brainly-design-infra-styleguide-sg-storybook-deploy-s3
...
.PHONY: build-brainly-design-infra-styleguide-sg-prod-deploy-s3
sg-prod-deploy-s3:
	echo build app $(APP_NAME) for $(ENVIRONMENT_NAME)
	yarn
	NODE_ENV=production node ./scripts/build-styleguide-s3.js

.PHONY: build-brainly-design-infra-styleguide-sg-prod-beta-release-deploy-s3

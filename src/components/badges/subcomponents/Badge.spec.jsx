...
describe('<Badge />', () => {
  it('renders without error', () => {
    const badge = shallow(<Badge>Test</Badge>);
  ...
  it("doesn't add additional classes when has default size", () => {
    const badge = mount(<Badge>Test</Badge>);
  ...
  it('adds class modifier when has small size', () => {
    const badge = shallow(<Badge size={BADGE_SIZE.SMALL}>Test</Badge>);

    expect(badge.hasClass('sg-badge--small')).toEqual(true);
  });

  it('adds class modifier when has larger size', () => {
    const badge = shallow(<Badge size={BADGE_SIZE.LARGE}>Test</Badge>);

...
import Subheading, {SUBHEADING_TYPE, SUBHEADING_SIZE} from '../Subheading';
...
[Text Bits ↓](#text-bit), [Headlines ↓](#headlines), [Subheadings ↓](#subheadings), [Body Text ↓](#body-text).
...
export const subheadingSizesMap = [
  {
    type: 'xsmall',
    fontSize: '14px',
  },
...
export const SubheadingExamples = () => {
  const sizeVariant = [];
  getValues(SUBHEADING_SIZE, false)
    .reverse()
    .forEach((size) => {
      let itemSize;
      ...
      sizeVariant.push(
        <TypeSpecimen key={size}>
          <Label>{size}</Label>
          <Subheading key={size} type={SUBHEADING_TYPE.H2} size={size}>
            Headline
          </Subheading>
        </TypeSpecimen>
      );
...
<Typeset>{SubheadingExamples()}</Typeset>

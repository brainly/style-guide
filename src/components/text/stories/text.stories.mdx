...
Each component has its own accessibility documentation containing a set of rules and usage examples to help implement accessible and readable text. The user can also adjust the spacing and font size to suit their own requirements using their browser settings. For this reason, the text should also be readable and not truncated when user sets line height to 1.5 times font size, spacing following paragraphs to (at least) 2 times font size, letter spacing to (at least) 0.12 times font size, word spacing to (at least) 0.16 times its size. To test that, apply following CSS rules:

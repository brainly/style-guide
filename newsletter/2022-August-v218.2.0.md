...
Did you ever want to try some new feature out, before making changes to the whole StyleGuide? Now you can! You can release the StyleGuide as beta version, which also includes assets.

To release a beta version, change the version in the package.json, so it contains `-beta.0` at the end and commit it. When you want to publish next version of beta, then increment the `.0` to `.1` and so on.

So our version should, for example, look like this: `218.2.1-beta.0`.

In addition it is always a good practice to add a git tag. We can add a tag with `git tag v218.2.1-beta.0`.

To make your life easier you can also run following command, so update of `package.json` and git tag will be made in one go:
